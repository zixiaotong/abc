...
public final class DateUtil {
	 ...
	 * 格林威治时间
	 */
	private static String gmttimetamPattern = "EE MMM dd HH:mm:ss zzz yyyy";
	 ...
	 * @return
	 */
	private static String showDateDetail(Date date) {
		Calendar target = DateUtil.dataToCalendar(date);
		Calendar today = DateUtil.dataToCalendar(DateUtil.getStartTimeOfCurrentDay());
		long intervalMilli = target.getTimeInMillis() - today.getTimeInMillis();
		int xcts = (int) (intervalMilli / (24 * 60 * 60 * 1000));

		if (target.getTime().getTime() > DateUtil.getStartTimeOfCurrentDay().getTime()) {
			switch (xcts) {
			case 0:
			...
			case 2:
				return DateUtil.AFTER_TOMORROW;
			case -1:
				return DateUtil.YESTERDAY;
			case -2:
				return DateUtil.BEFORE_YESTERDAY;
			default:
				int dayForWeek = 0;
			...
			return DateUtil.formatDate(date, DateUtil.timestampPattern);
		}
	}

	private static SimpleDateFormat getTimestampMonthPatternFormat() {
		ThreadLocal<SimpleDateFormat> tl = dateFormatPool.get(timestampMonthPattern);
		if (null == tl) {
	...
	private static SimpleDateFormat getDateFormatGMT() {
		ThreadLocal<SimpleDateFormat> tl = dateFormatPool.get(gmttimetamPattern);
		if (null == tl) {
			synchronized (dateFormatLock) {
	...
	private static SimpleDateFormat getSimplifyExtendTimestampFormat() {
		ThreadLocal<SimpleDateFormat> tl = timestampFormatPool.get(simplifyExtendTimestampPattern);
		if (null == tl) {
			synchronized (timestampFormatLock) {
				...
				if (null == tl) {
					tl = new ThreadLocal<SimpleDateFormat>() {
						protected synchronized SimpleDateFormat initialValue() {
							return new SimpleDateFormat(simplifyExtendTimestampPattern);
						}
					};
	...
	private static SimpleDateFormat getSimplifyMonthFormat() {
		ThreadLocal<SimpleDateFormat> tl = timestampFormatPool.get(simplifyMonthPattern);
		if (null == tl) {
			synchronized (timestampFormatLock) {
	...
	public static boolean isOutOfDate(String time, String start_time, String over_time) throws ParseException {
		long timeLong = new SimpleDateFormat(timestampPattern).parse(time).getTime();
		long ckStartTimeLong = new SimpleDateFormat(timestampPattern).parse(start_time).getTime();
		long ckOverTimeLong = new SimpleDateFormat(timestampPattern).parse(over_time).getTime();
	...
	public static void main(String args[]) throws Exception {
		...
		Date a = formatTimestampToDate("2019-12-02 09:30:30");
		String s = showDateDetail(a);
